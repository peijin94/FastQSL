!    ifort -o qfactor.x qfactor_stretch.f90 -fopenmp -O3 -xHost -ipo
! gfortran -o qfactor.x qfactor_stretch.f90 -fopenmp -Ofast -march=native
! -O3, -xHost, -ipo, -Ofast, -march=native are for a better efficiency

include  'trace_bline_stretch.f90'
include  'trace_scott_stretch.f90'
...
real:: nxx, nxy, nyx, nyy, vp(0:2), q0, q_perp0
...
real:: bp(0:2), vp(0:2), br(0:2), rs(0:2), re(0:2), &
	twist0, bn_0, bn_s, bn_e, line_length, q0, q_perp0, incline
...
if ((q(i,j) .ge. 10.0) .and. (q(i,j)/ bnr(i,j) .ge. 10.0) .and. (sum(boundary_mark) .eq. 0)) then
	...
	if ( ( rbs .eq. rbsa1) .and. ( rbs .eq. rbsa2) .and. &
		...
		q(i,j) = (nxx*nxx + nxy*nxy + nyx*nyx + nyy*nyy) * bnt_tmp /((2.*delta1)**4.)
	endif
else
	q(i,j)=qtmp(i,j)
endif
...
deallocate(volume, xa, ya, za, dxa, dya, dza)
