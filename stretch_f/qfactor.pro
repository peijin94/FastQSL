PRO qfactor, bx, by, bz, stretchFlag=stretchFlag, xa=xa, ya=ya, za=za, $
...
;   or a box volume, given a 3D magnetic field with uniform or stretched grids
; 
;   For details see:
;     Liu et al. (2016, ApJ)  
;
...
;
;      ifort -o qfactor_stretch.x qfactor_stretch.f90 -fopenmp -O3 -xHost -ipo
;   gfortran -o qfactor_stretch.x qfactor_stretch.f90 -fopenmp -Ofast -march=native
;
;   -O3, -xHost, -ipo, -Ofast, -march=native are for a better efficiency
;  please specify the path of qfactor.x, qfactor_stretch.x at the line of "spawn, 'qfactor.x' " in this file, 
;  or move qfactor.x, qfactor_stretch.x to the $PATH (e.g. /usr/local/bin/) of the system
;
;
; INPUTS               
...
;   
; OPTIONAL INPUTS  
;   stretchFlag: Bx, By, Bz are on stretched grids; default is 0 (uniform)
;
;   xa, ya, za:  coordinates of grids in 1D arrays, when stretchFlag invoked. 
;                The size of xa, ya, za must be consistant with the size of the 3D magnetic field, and the values in x[yz]a should increase by order
;
...
;
;   delta:      grid spacing; default is 1/factor; if it is set, factor will be ignored
;
...
;       3D magnetic field + some 2D arrays + 3D CurlB field (same as the occupation of the 3D magnetic field, when twistFlag and vFlag) +
;       grad_unit_vec_Bfield (3 times as the occupation of the 3D magnetic field, when scottFlag or vflag invoked)
;
;
; OUTPUTS:
...
if keyword_set(stretchFlag) then stretchFlag=1B else stretchFlag=0B
...
if (stretchFlag) then begin
	...
	message, 'the size of xa, ya, za must be consistant with the size of the 3D magnetic field'
endif

if (stretchFlag) then begin
	if ~keyword_set(xreg) then xreg=[xa(0), xa(nxa-1)]
	if ~keyword_set(yreg) then yreg=[ya(0), ya(nya-1)]
...
if ~stretchFlag then magnetogram=Bz[*,*,0]
...
tnow=systime(1)
if stretchFlag then spawn, 'qfactor_stretch.x' else spawn, 'qfactor.x' ; if not known by the system, specify the path
tend=systime(1)
...
if (~no_preview) then begin
	...
	if (stretchFlag) then begin	
		if csflag then plots,([xreg[0],xreg[1]]-xa(0))/delta_mag, ([yreg[0],yreg[1]]-ya(0))/delta_mag, /dev $
